...
unsigned int TextureImporter::ImportTexture(const std::string& path,
	unsigned int width,
	unsigned int height)
{

	unsigned char* data;
	LoadBMPFile(path, width, height, data);
...
void TextureImporter::LoadBMPFile(const std::string& path,
	unsigned int& width,
	unsigned int& height,
	unsigned char*& data)
{
